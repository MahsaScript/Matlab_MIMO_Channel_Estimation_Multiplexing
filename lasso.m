...
%                      cvpartition of type 'leaveout'. You cannot use a
%                      cvpartition of type 'holdout' with the IsCustom
%                      property as true. When 'CV' is 'resubstitution',
%                      LASSO uses X and Y both to fit the model and to estimate
%                      the mean squared errors, without cross-validation. 
...
%                      If 'CV' is 'resubstitution', a cvpartition of type
%                      'resubstitution', or a cvpartition of type 'kfold' with
%                      the IsCustom property as true, then 'MCReps' must be 1.
%                      If 'CV' is a cvpartition of type 'holdout', then
%                      'MCReps' must be greater than one.
...
%                      matrix, even if the UseCovariance argument is true.
...
elseif isa(cvp,'cvpartition')
    if strcmpi(cvp.Type,'resubstitution')
        cvp = 'resubstitution';
    elseif strcmpi(cvp.Type,'leaveout') % both custom and non-custom
        error(message('stats:lasso:InvalidCVtype'));
    elseif strcmpi(cvp.Type,'holdout') && cvp.IsCustom
        error(message('stats:lasso:InvalidCustomCVType'));
    elseif strcmpi(cvp.Type,'Kfold') && cvp.IsCustom && mcreps ~= 1
        error(message('stats:lasso:InvalidMCRepsCustom'));
...
active = false(1,P);
mspe = zeros(1,nLambda); % Mean Prediction Squared Error

% Run coordinate descent only when at least one lambda is less
% than lambdaMax
if all(lambda>=lambdaMax)
    Intercept = repmat(muY,1,nLambda);
    mspe = repmat(nullMSE,1,nLambda);
else
% At least one lambda is less than lambdaMax
if useCov
    wXTy = wX0.'*Y0;
    wCov = wX0.'*X0;
    sumSquaresCols = [];
...
for i = 1:nLambda
    ...
    if ~isempty(shrinkFactor) && ~isempty(lamLast)
        % Save recomputing X0.^2 when computing the subsequent shrinkFactor
        shrinkFactor = shrinkFactor + (lam-lamLast)*(1 - alpha);
    else
        % First time calculating shrinkFactor
        if withConstant
            if standardize
                if observationWeights
    ...
    if useCov
        if ~observationWeights
            mspe(i) = 1/N * (wyNorm + b'*wCov*b-2*b'*wXTy);
        else
            % This line relies on the weights having been normalized.
            mspe(i) = wyNorm + b'*wCov*b-2*b'*wXTy;
        end
    else
        if ~observationWeights
            mspe(i) = mean(r.^2);
...
% LocalWords:  DFmax DF UCI df bls Zou Hastie XnotaReal DMatrix YnotaConformingVector okrows
% LocalWords:  numlambda lambdaratio dfmax mcreps usecovariance cvp InvalidCVforX InvalidCVtype
% LocalWords:  cdescentCycle

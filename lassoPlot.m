...
%LASSOPLOT Plot coefficient values or goodness of fit of lasso or elastic
%net fits.
...
%  representing a set of coefficients estimated by LASSO or LASSOGLM using
%  a single penalty term Lambda.  AXH is an axes handle that gives access
%  to the axes used to plot the coefficient values B.  FIGH is a handle to
%  the figure window.
%
%  [AXH,FIGH] = LASSOPLOT(B) plots all the coefficient values contained in
%  B against the L1-norm of B. The L1-norm is the  sum of the absolute
%  value of all the coefficients. The plot is also annotated with the
%  number of non-zero coefficients of B ("df"), displayed along the top
%  axis of the plot.
%
%  [AXH,FIGH] = LASSOPLOT(B,PLOTDATA) creates a plot with contents
%  dependent on the type of PLOTDATA.
%
%     If PLOTDATA is a vector, then LASSOPLOT(B,PLOTDATA) uses the values
%     of PLOTDATA to form the x-axis of the plot, rather than the L1-norm
%     of B. In this case, PLOTDATA must have the same length as the number
%     of columns as B.
%
%     If PLOTDATA is a struct, then LASSOPLOT(B,PLOTDATA,'PlotType',val)
%     allows you to control aspects of the plot depending on the value of
%     the optional argument 'PlotType'. The possible values for 'PlotType'
%     are:
%
...
%                  value of lambda, plus error bars for the estimate. For
%                  fits performed by LASSO, the goodness of fit is MSE, or
%                  mean squared prediction error. For fits performed by
%                  LASSOGLM, the goodness of fit criterion is Deviance, the
%                  goodness of fit measure used by GLMFIT. The 'CV' plot
%                  also indicates the value for lambda with the minimum
...
%                  plot type is valid only if PLOTDATA was produced by a
%                  call to LASSO or LASSOGLM with cross-validation enabled.
% 
%
%  LASSOPLOT(...,'PARAM',val)
%
%     'PredictorNames' A string array or cell array of strings to label
%     each of the coefficients
%                      of B. Default: {'B1', 'B2', ...}. If PREDICTORNAMES
%                      is of length less than the number of rows of B, the
%                      remaining labels will be padded with default values.
%
...
if nargin > 1
    plotData = convertStringsToChars(plotData);
end
...
if isequal(plotType,'CV')
    ...
    % Move the axes above its siblings.
    parent = ax1.Parent;
    siblings = parent.Children;
    parent.Children = [ax1; setdiff(siblings, ax1)];

    % Ensure axes with "df" values does not respond to mouse actions.
    set(ax2,'hittest','off');
    ...
    % Place title on ax2 so it will be positioned above the df values
    title(ax2,theTitle);
    title(ax1,theTitle,'Visible','off');
...
axisPosition = get(ax1,'InnerPosition');
left = axisPosition(1);
...
width  = width * 0.90;
height = height * 0.75;
set(ax1,'InnerPosition',[left bottom width height]);
...
ax2 = axes('Parent',get(ax1,'Parent'),...
    'Units', get(ax1, 'Units'), ...
    'InnerPosition',get(ax1,'InnerPosition'), ...
    'Color','none', ...
    'XAxisLocation','top', ...
    'XDir', get(ax1,'XDir'), ...
    'XLim', get(ax1,'XLim'), ...
...
% The title is drawn on ax2 because that puts it above the df text. However,
% ax1 is the returned handle. If the title text gets changed on the
% returned handle, copy that to ax2.
titleListener = addlistener(ax1.Title,{'String'},'PostSet',...
    @(src,evt) copyTitle(ax1,ax2));

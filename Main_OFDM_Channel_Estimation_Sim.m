...
%Moduluation method: BPSK, QPSK, PSK, 16QAM, 32QAM, 64QAM,
...
%% Read the signal and convert it into binary format.
im = randn(512,512,3);
im_bin = dec2bin(im(:))';
im_bin = im_bin(:);
...
grid on
weights=[0;2;0;-3;0];
y=X_hatt*weights+randn(100,1)*0.1; % small add noise
lambda=1e-3;
[B, FitInfo]=lasso(X_hatt,y,'CV',10,'PredictorNames',{'x1','x2','x3','x4','x5'}, 'Lambda',lambda, 'Intercept', false);
MMSE = FitInfo.IndexMinMSE;
subplot(2,1,2);
scatter(X_hatt,y);
hold on;
